...
这是一套指导我们编写JavaScript的行为规范，她来自于我们讨论和参考行业内文档所形成的，为了保证代码风格保持一致，让代码可以更容易被理解和被维护，同时为了避免一些显性的bug提供最佳实践。
...
##### [强制] 必须独占一行。`//` 后跟一个空格，缩进与下一行被注释说明的代码一致,在注释前插入空行。
...
##### [建议] 如果通过 `if` 和 `else` 使用多行代码块，把 `else` 放在 `if` 代码块关闭括号的同一行。
